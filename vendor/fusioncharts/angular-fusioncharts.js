    ...
    a.directive("fusioncharts", ["$http", function(a) {
        return {
            ...
            link: function(a, b, c) {
                ...
                if (c.events) {
                    for (m = c.events.split("."), n = m.length, g = a.$parent, e = 0; n > e; e += 1)
                        g = g && g[m[e]];
                    if (g)
                        for (h in g)
                ...
                for (e = 0; e < q.length; e++)
                    f = q[e], f.match(/^on/i) && (h = f.slice(2).toLowerCase(), p[h] = a.$parent[c[f]]);
                j = {
                    type: c.type,
                    width: c.width,
                    height: c.height,
                ...
                for (i in o.NDCObserver)
                    attrConfig = o.NDCObserver[i], (attrConfig.ifExist===!1 || c[i]) && c.$observe(i, attrConfig.observer);
                ...
                for (i in o.DCObserver)
                    attrConfig = o.DCObserver[i], l = c[i], l ? (c.$observe(i, attrConfig.observer), v[i] = l, "json" === j.dataFormat && "object" == typeof j.dataSource && (j.dataSource[i] = JSON.parse(l))) : attrConfig.ifExist===!1 && c.$observe(i, attrConfig.observer);
                if (c.config) {
