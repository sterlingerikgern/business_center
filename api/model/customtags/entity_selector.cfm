...
<div class="col-sm-12">
	<div class="portlet">
		<div class="portlet-header">
			...
			<cfif attributes.entitytypeid GT "0"> 
		...
		<div class="row"  id="filters_keyword">
			<div class="col-sm-12">
		...
		<cfif attributes.entitytypeid GT "0"> 
			<div class="row" id="filters_include" style="display:none"></div>
		...
		<div class="row" id="filters_other" style="display:none">
			...
			<div class="col-md-4 col-sm-6 col-xs-6">
				<select name="filter_ACTIVITYDAYRANGE" id="filter_ACTIVITYDAYRANGE" class="form-control select2-input input-sm">
					<option value=""></option>
					<option value="-180:-90" class="inactive">Inactive 90 to 180 days</option>
					<option value="-89:-60" class="inactive">Inactive 60 to 89 days</option>
					<option value="-59:-30" class="inactive">Inactive 30 to 59 days</option>
					<option value="-29:-15" class="inactive">Inactive 15 to 29 days</option>
					<option value="0:0" class="active">Never Active</option>
		...
		<cfelseif attributes.entitytypeid IS "11"> <!--- this is a flag value --->
		
			<div class="row" id="filters_other" style="display:none">
				<div class="col-md-4 col-sm-6 col-xs-6">
					 <div id="startDateDiv" class="input-group date" data-auto-close="true" data-date="#DateFormat(now()-90, "mm-dd-yyyy")#" data-date-format="mm-dd-yyyy" data-date-autoclose="true">
						<input class="form-control" type="text" name="filter_STARTDATE" id="filter_STARTDATE" placeholder="Start Date">
						<span class="input-group-addon"><i class="fa fa-calendar"></i></span>
					</div>
				</div>
				<div class="col-md-4 col-sm-6 col-xs-6">
					<div id="endDateDiv" class="input-group date" data-auto-close="true" data-date="#DateFormat(now(), "mm-dd-yyyy")#" data-date-format="mm-dd-yyyy" data-date-autoclose="true">
						<input class="form-control" type="text" name="filter_ENDDATE" id="filter_ENDDATE" placeholder="End Date">
						<span class="input-group-addon"><i class="fa fa-calendar"></i></span>
					</div>
		 	</div>
		</cfif>
...
var startDate = $.parseJSON(localStorage.getItem("startDate"));
var endDate = $.parseJSON(localStorage.getItem("endDate"));
...
<cfif IsDefined("caller.rc.lockToEntityid")>

	<cfinvoke component="#application.cfcs["perkz_global"]#" method="GetEntityParentIDs" returnvariable="rc.parentList" >	
		<cfinvokeargument name="entityID" value="#caller.rc.lockToEntityid#">
	</cfinvoke>
	selectedIncludes = [];
	
	var parentsList = '#rc.parentList#';
	parentsList = parentsList.replace(/\s+/g, '');
	
	var parentsListArray = parentsList.split(',');	
	parentsListArray.push('#caller.rc.lockToEntityid#');
	
	//window.console && console.log(selectedIncludes);
	for(i in parentsListArray) {
		//window.console && console.log($.inArray( parentsListArray[i], selectedIncludes ));
		if($.inArray( parentsListArray[i], selectedIncludes )  == -1){
		
			selectedIncludes.push(parentsListArray[i]);
...
$displayFilters = function(){
	...
	if(advancedFiltersVisibility){
		...
		//$('##filters_keyword').show();
		$('##filters_other').hide();
		$('##filters_include').hide(); 
		$('##filters_exclude').hide();
	}
 		
 	if($('##filters_exclude > .col-sm-6:visible').length == 0) {
			$('##filters_exclude').hide(); 
		}	
	$saveState();
}
...
$fillOptions = function(in_ex, addedID, removedID) {
	...
	for (i=lowestLevel;i<=highestLevel;i++) { 
			...
			//window.console && console.log(selectedIncludes);
				...
				if ( hByLevel[j].ENTITYLEVEL == i   <!--- if items belong in this level --->
					 ...
					 && $.inArray(hByLevel[j].PARENTENTITYID, selectedIncludes ) >= 0  <!--- is my parent selected in includes --->
						...
						if($.inArray(hByLevel[j].ENTITYID, selectedIncludes) >= 0) {
							crumbs.push(hByLevel[j].ENTITYNAME);
		...
		if($('##include_'+i).children().length > 0) {
			$('##div_include_'+i).show();
			$('##include_'+i).select2({placeholder: "Include items below....",formatResult: formatOptions,formatSelection: formatOptions, containerCssClass: "marginbottom",minimumResultsForSearch: "20"});
	...
	for (i=lowestLevel;i<=highestLevel;i++) { 
		...
		if($('##exclude_'+i).children().length > 0) {
			$('##div_exclude_'+i).show();
			$('##exclude_'+i).select2({placeholder: "Exclude items below....",formatResult: formatOptions,formatSelection: formatOptions, containerCssClass: "marginbottom",minimumResultsForSearch: "20"});
	...
	if(advancedFiltersVisibility){
		$('##filters_exclude').show();
		if($('##filters_exclude').find('.col-sm-6:visible').length == 0) $('##filters_exclude').hide();
	}

	$('##filters_include').show();
	if($('##filters_include').find('.col-sm-6:visible').length == 0) $('##filters_include').hide();
}	<!--- end fillOptions() --->	
...
for (i=lowestLevel;i<=highestLevel;i++) {
	$('##filters_include').append($("<div class='col-md-4 col-sm-6 col-xs-6' id='div_include_"+i+"' level='"+i+"' style='display:none'><select name='include_" + i +  "' id='include_" + i +"' level='"+i+"' multiple class='form-control select2-input'></select></div>"));	
	
	$('##filters_exclude').append($("<div class='col-md-4 col-sm-6 col-xs-6' id='div_exclude_"+i+"' level='"+i+"' style='display:none'><select name='exclude_" + i +  "' id='exclude_" + i +"' level='"+i+"' multiple class='form-control select2-input'></select></div>"));
...
<cfif attributes.entitytypeid IS "11"> 
...
$('##filter_STARTDATE').val(startDate);
$('##filter_ENDDATE').val(endDate);
